...
for(let i = 1; i < tr.length; i++){
    ...
    const table = {};
    for(let j = 0; j < heads.length; j++)table[heads[j]] = cells[j]?.textContent || "";
    tables.push(table);
}
tables.pop();
...
thead.addEventListener("click", (e)=>{
    ...
    let sortedTables = [];
    if (header.textContent === heads[0]) sortedTables = tables.sort((a, b)=>{
        return a[heads[0]].localeCompare(b[heads[0]]);
    });
    if (header.textContent === heads[1]) sortedTables = tables.sort((a, b)=>{
        return a[heads[1]].localeCompare(b[heads[1]]);
    });
    if (header.textContent === heads[2]) sortedTables = tables.sort((a, b)=>{
        return +a[heads[2]] - +b[heads[2]];
    });
    if (header.textContent === heads[3]) sortedTables = tables.sort((a, b)=>{
    ...
    sortedTables.forEach((person)=>{
        const { Name: personName, Position: position, Age: age, Salary: salary } = person;
